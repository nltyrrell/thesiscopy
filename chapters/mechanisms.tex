...
With the column model we were able to model a process by which SST variability 
became amplified over land, leading to a land/ocean contrast. The column model 
also showed that the magnitude of the land/ocean contrast can be controlled by 
the evaporative fraction of the land surface. A comparison of the results with a 
GCM showed that for many continental regions SST signals can become amplified 
but cannot be explained by the column model.  Instead we used another simplified 
atmospheric model -- the GREB (Globally Resolved Energy Balance) model -- to 
deconstruct the land response to an SST forcing.  The boundary conditions of the 
GREB model include SSTs, winds, soil moisture, and clouds. Starting with a 
climatology, these variables can be changed individually and in combination to 
recreate
