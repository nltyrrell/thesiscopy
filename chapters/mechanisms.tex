...
the maximum, mininum and median SST forcing. The full profile is shown in (a) 
...
the LCL as seen in the single coumn model. There is some indication in the 
...
flatter profile, and the height of the local mininum can indicates the location 
...
the suitability of this simplifed model when comparing the results to GCMs or 
obserations.  Figure \ref{fig:tdm_psi} (a)-(d) shows how the low level 
circulation scales with evaporative fraction. A drier land surface results in a  
stronger low level circulation.  Refering back to Table \ref{tab:tdm_Rlo}, 
...
\subsection{Values of land ocean contrast for varying land parameters}
...
\begin{figure}[ht]
	...
	\caption{Land/ocean contrast ($R_{l/o}$) in multi column model for a range 
		...
		relative humidity at each latitude and evaporative fration, (d) uses the 
	first column of the meridionally aligned model to calculate $R_{l/o}$ 
instead of the areal mean across all columns.}
\label{fig:tdm_Rlo}
\end{figure}
