...
\subsection{Delayed ocean and land response}
\label{xcor}
...
explored with a singole and multi column model.
...
\label{sec:mcm_prof_trh}

In section \ref{sec:scm_prof} the atmospheric profiles of $\theta$ for the 
single column indicated that a changing LCL height over land as well as a 
deviation between land and ocean lapse rates above the LCL can lead to 
$R_{l/o}>1$. We will know look at profiles of $\theta$ in the multi column 
model. The potential importance of a changing LCL height motivated an increase 
in the vertical resolution of the model. The vertical resolution was increased 
from 42 to 85 levels, and a non-uniform grid with a resolution of 
\SI{5}{\hecto\pascal} between \SIrange{980}{800}{\hecto\pascal} and 
\SI{10}{\hecto\pascal} between \SIrange{800}{650}{\hecto\pascal}. A fine 
resolution grid around the height of the LCL was intended to prevent step 
changes in the height of the LCL.  Figure \ref{fig:tdm_prof} shows three 
temperature profiles from a run with an evaporative fraction of 0.05 and an 
$R_{l/o}$ of 1.13. The three profiles are the maximum, minimum and median SST 
forcing. The full profile is shown in (a) with a close up in (b). There are two 
features to look for; a change in the gradient of $\theta$ in the ocean forced 
column between the ocean LCL and land LCL as per the \citet{Byrne2013a} theory, 
and any significant change in the height of the LCL as seen in the single column 
model. There is some indication in the figure of a small change in the LCL over 
land but it is not as pronounced as in the single-column, and a change in 
gradient of $\theta$ is difficult to determine in Figure \ref{fig:tdm_prof}.  
Instead we look at Figure \ref{fig:tdm_dthdp}, which shows the vertical gradient 
of $\theta$, $D\theta/Dp$, to determine any significant features.  An increase 
in the magnitude of $D\theta/Dp$ (i.e. more negative) would indicate a flatter 
profile, and the height of the local minimum can indicates the location of the 
LCL. In Figure \ref{fig:tdm_dthdp}, the gradient of $\theta$ in the ocean column 
above the LCL becomes more negative with increasing SST which is the expected 
result according to the Byrne and O'Gorman theory.  However, there is also an 
unusual feature; from \SIrange{900}{800}{\hecto\pascal} the land profile is 
above, or cooler than, the ocean profile. This region of warmer ocean/cooler 
...
columns. The profiles for two SSTs are plotted, the warmest and coolest of the 
runs, as the results are similar for a range of SSTs.  From the surface we see 
the much higher relative humidity over the ocean forced column then a sudden 
drop at around \SI{970}{\hecto\pascal}, to the extent that the ocean forced 
column becomes drier than the land columns. The dry region, from 
\SIrange{940}{800}{\hecto\pascal} coincides with the steeper $\theta$ lapse 
rate. As this run has only one ocean column and three land columns the drying 
may be related to the model geometry.  To test if the geometry is responsible 
for the dry region, we look at a run with three ocean and three land columns.  
Figures \ref{fig:tdm_rhprof} (b)-(d) all show a similar drying in the ocean 
forced column, these plots are also focusing on the atmosphere below 
\SI{800}{\hecto\pascal}.  There is a robust response with the three ocean and 
three lands, implying the drying is not a function of the geometry or due to the 
number of ocean forced columns.  Figures \ref{fig:tdm_rhprof} (b), (c) and (d) 
show three difference evaporative fractions, 0.05, 0.10 and 0.20. The 
evaporative fraction has a clear effect on the relative humidity in the land 
column, but the amount of drying in the ocean forced columns is largely 
insensitive to the evaporative fraction or relative humidity of the land 
columns. For example comparing Figure \ref{fig:tdm_rhprof} b), c) and d) which 
have evaporative fractions of 0.05, 0.10 and 0.20  respectively, the near 
surface humidity in the land columns changes from 45\% to 60\%, however, the 
response of the ocean forced columns is very similar.  The difference in the dry 
region is at most 5\%.
...
\citet{Byrne2013a}, could contribute to the land/ocean contrast. Figure 
\ref{fig:tdm_dthdp} shows a decrease in the gradient of $\theta$ profiles with 
warmer SSTs.  However, the lapse rate over the ocean is being affected by a 
strong drying, indicated in the relative humidity profiles in Figure 
\ref{fig:tdm_rhprof}. The dryness of the surface, and hence the near surface 
layers, was shown to influence the magnitude of $R_{l/o}$ in section 
\ref{sec:robust_rlo}, so a drying signal in the ocean columns may also effect 
the values of $R_{l/o}$. Next we investigate the drying in the ocean columns by 
looking at the circulation response in the column model.
...
vertical velocity, $\omega$ as shown in Figure \ref{fig:tdm_circ}, and 
circulation using the streamfunction $\psi$ of vertical and zonal velocities, in 
Figure \ref{fig:tdm_psi}.  There weren't any significant changes in the strength 
or patterns of the circulation as SSTs were increased and decreased around a 
mean value, so for these variables we use the mean across a range of SSTs.  The 
profile of $\omega$ in Figure \ref{fig:tdm_psi} shows convection in the ocean 
forced columns -- positive values of $\omega$ -- which is balanced with sinking 
motion in the land columns.  Large values of $\omega$ occur in the ocean forced 
columns in the area where the drying occurs, around 
\SIrange{970}{800}{\hecto\pascal}, but the cause of the drying is not clear from 
this figure.  The $\psi$ profile in Figure \ref{fig:tdm_psi} better demonstrates 
the flow in the model. Positive values of $\psi$ indicate clockwise flow 
centered at \SI{850}{\hecto\pascal} in the ocean forced column nearest the land.  
This low-level circulation is responsible for transporting dry air from the land 
columns over the ocean.
...
We have seen that while the multi column model has a low level circulation that 
dries the ocean forced column, it is still able to simulate aspects of the 
...
fractions and also vary the latitude to build a picture of the range of 
$R_{l/o}$ values.

The model was setup with 3 ocean and 3 land columns and initially was run in a 
zonally orientated domain, with each column spanning \SI{5}{\degree} of latitude 
and longitude.  The change in latitude involved choosing an
appropriate mean SST value, solar zenith angle and Coriolis parameter. This was 
done for latitudes \SI{0}{\degree}, \SI{10}{\degree}, \SI{20}{\degree}, 
\SI{30}{\degree}, \SI{40}{\degree}, \SI{50}{\degree} \& \SI{60}{\degree}.
The model was forced by varying the SSTs by \SI{1.2}{\kelvin} in increments of 
\SI{0.2}{\kelvin} above and below the SST appropriate each latitude, and for 
each increment the model was run to equilibrium. This was repeated for 
evaporative fractions from 0.02 to 1.0, and $R_{l/o}$ was calculated for each 
latitude and evaporative fraction.	The results, in Figure \ref{fig:tdm_Rlo} (a) 
show the dependence of $R_{l/o}$ on evaporative fraction, but also a largely 
monotonic decrease with latitude, excepting the increase for some evaporative 
fractions from \SI{0}{\degree} to \SI{10}{\degree}.  The same experiments were 
also performed using a meridionally orientated domain, shown in Figure 
\ref{fig:tdm_Rlo} (b).  For this geometry the columns with prescribed SSTs were 
nearer to the equator and the land columns extended towards the poles. All 
surface values for the zonal orientation are calculated as the mean of the three 
ocean or land values.  However, as each column spans \SI{5}{\degree} of 
latitude, in the meridional orientation we use the land column nearest to the 
equator to calculate $R_{l/o}$. Figure \ref{fig:tdm_Rlo} (b) shows the same 
tendencies as \ref{fig:tdm_Rlo} (a).
...
To understand the reduction in the $R_{l/o}$ with increasing latitude we will 
look at the near surface (lowest model level) relative humidity over land, which 
is shown in Figure \ref{fig:tdm_rh}. The gradient of relative humidity along a 
constant latitude due to varying evaporative fraction is clear, and increasing 
relative humidity corresponds to decreasing $R_{l/o}$ as expected.  The relative 
humidity also decreases with increasing latitude, due to reduced transport of 
moisture from the ocean forced columns, which will be further discussed in 
section \ref{mech_compare}.  For the gradient of relative humidity across 
latitudes at a constant evaporative fraction, decreasing relative humidity is 
associated with decreasing $R_{l/o}$, which is the inverse of the relationship 
with a constant latitude.  This indicates that the value of $R_{l/o}$ is a not 
only a function of evaporative fraction but is also dependent on transport from 
ocean forced columns to land columns.
...
In Figure \ref{fig:tdm_rh} we saw that relative humidity decreased with 
...
understand the changes. Figure \ref{fig:tdm_psi_lat} shows the circulation 
response for the zonally orientated column model. The circulation is not 
significantly affected by small SST changes, hence this figure shows the mean 
across the range of SSTs. As the latitude increases there is a change in the 
circulation.  At low latitudes the model is essentially one cell, with rising 
air over the ocean forced columns and sinking over the land.  As the Coriolis 
parameter increases with latitude the circulation is inhibited, and the single 
overturing cell begins to break down at \SI{20}{\degree}. By \SI{30}{\degree} 
smaller cells have formed between each column. This will reduce the transport of 
heat and moisture from the ocean forced columns to the land columns, resulting 
in the reduced relative humidity and reduced values of $R_{l/o}$ seen with 
increasing latitude.
...
In the single-column model the interaction between the cloud and radiation 
schemes lead to chaotic behaviour and instability in the model (section 
\ref{instability}). However, as this interaction may still be an important 
aspect of the land/ocean contrast we repeated the experiments used to create 
Figure \ref{fig:tdm_Rlo} with the interaction between clouds and radiation 
turned on. The plot in Figure \ref{fig:tdm_Rlo_ic} is broadly similar to Figure 
\ref{fig:tdm_Rlo} in terms of the response of $R_{l/o}$ to the changing 
evaporative fraction, in that a drier surface tends to have a higher $R_{l/o}$.  
The tendency of $R_{l/o}$ with latitude is different, we don't see a reduction 
with increasing latitude like in Figure \ref{fig:tdm_Rlo}.  The main feature, 
however, is the randomness present in the results.  It seems at certain 
latitudes the model behaves strangely, namely at \SI{10}{\degree} where 
$R_{l/o}$ values are very low and \SI{40}{\degree} where they are very high.  
While we expect that the cloud and radiation interaction is likely to be an 
important process in the land/ocean contrast, this single or multi-column model 
setup may not be a suitable method of investigating it.
...
In this section we have studied the land/ocean contrast in a 
radiative-convective model setup as rows of columns with ocean or land surface 
boundary conditions. The multi column model produces values of $R_{l/o}$ similar 
to observations and GCMs, albeit on the lower end and with small evaporative 
fractions, and there similarities in the vertical structure. In the next section 
the multi column model values of $R_{l/o}$ will be compared directly with the 
ACCESS GCM.

\section{Comparison of Simplified Models and GCM results}
\label{mech_compare}

In section \ref{sec:rlo_lat} a large range of $R_{l/o}$ values were generated by 
by varying the latitude and evaporative fraction. These values allow for a 
direct comparison to GCM results.  Using the ENSO-pacemaker experiment we can 
calculate an $R_{l/o}$ value for each individual land grid box, see Figure 
\ref{fig:tdm_gcm_compare} a). We do this by comparing the grid box surface 
...
most appropriate for comparing to the zonally orientated multi column model, 
however, due to the tropical Pacific forcing of the pacemaker experiment, a grid 
box $R_{l/o}$ calculated using mean tropical SSTs gave the most reasonable 
results.

The next step is to generate a comparative map using the multi column column 
model results. For each GCM grid box we find the latitude and evaporative 
fraction of the surface, the evaporative fractions used are from the 
ENSO-Pacemaker experiment and are shown in Figure \ref{fig:gcm_ef}. For each 
grid box we find the value of $R_{l/o}$ that was generated with the multi column 
model for the corresponding latitude and evaporative fraction, and apply the 
column model $R_{l/o}$ value to the GCM map.  The result is shown Figure 
\ref{fig:tdm_gcm_compare} b).  For example, for a point in central Australia at 
a latitude of \SI{23}{\degree} and evaporative fraction of 0.1 we use the 
$R_{l/o}$ from the multi column model at latitude \SI{20}{\degree} and 
evaporative fraction 0.1 and assign that grid box a value of 1.12, thereby 
building a global map of the expected land surface temperature response based on 
the results of the two dimensional column model.

\begin{figure}[ht]
	\includegraphics[width=0.9\textwidth]{{thesis_mean_ef}.eps}
	\caption{Mean evaportive fraction for the ENSO-Pacemaker experiment.}
\label{fig:gcm_ef}
\end{figure}


The multi column model required very low evaporative fractions to elicit an 
...
similar between the multi column model and GCM results. The two plots in Figure 
\ref{fig:tdm_gcm_compare} share some broad similarities in the regional changes 
of $R_{l/o}$, but also have many difference in both the magnitude and patterns 
of $R_{l/o}$.
...
It is evident that the values of $R_{l/o}$ generated by the multi column model 
for a given evaporative fraction are too low. The only two large regions of the 
generated map with an $R_{l/o}>1$ are Saharan Africa and central Australia, 
although in both these regions the pattern of the land response differs from the 
GCM.  For example, in Saharan Africa the generated map shows a very consistent 
$R_{l/o}$ from western coast of Africa to the Persian gulf, as this region is of 
a similar dryness. In contrast, the GCM results only have an $R_{l/o}>1$ for the 
western half of the region.  Likewise in south-eastern Australia the GCM has a 
higher $R_{l/o}$ whereas the multi column model has lower values for that 
region.  Another area of interest is tropical South America, here the GCM has 
high $R_{l/o}$ values and a distinctive pattern with a very high "blob" over the 
Amazon basin, none of which is present in the multi column model.
...
\section{Mechanisms of the Regional Response of continental surface temperatures 
to ocean forcing.}
\label{mech_greb}
