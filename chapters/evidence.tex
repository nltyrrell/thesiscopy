...
convective model doesn't have inherent interannual variability. Variability was 
represented with imposed SSTs. To create anomalies on the order of 1-2K, SSTs 
were varied around a mean value, chosen to represent a given latitude. In 
addition to controlling the SSTs and land evaporative fraction, a number of 
different geometries of land and ocean columns were investigated.  The model 
domain can be orientated zonally or meridionally, and the number of ocean and 
land points can be altered. For each imposed SST anomaly the model was run to 
equilibrium and the resulting land temperature was campared with the forcing 
SST. The results for a domain with three ocean columns and three land columns is 
shown in figure \ref{fig:tdm_Rlo} with the values shown in table 
\ref{tab:tdm_Rlo}. The results are similar to the single-column model in 
weak-temperature gradient mode; the value of $R_{l/o}$ is dependant primarily on 
the evaporative fraction. The domain setup also influences $R_{l/o}$ as it can 
affect the near surface humidity. When the model is run with three ocean and 
just one land column the land response is suppressed relative to the SST 
forcing, leading to values of $R_{l/o}$ of around 0.7. $R_{l/o}$ is also 
sensitive to the mean SST value and latitude. As the mean SST and latitude 
decreases so too does $R_{l/o}$.
...
\begin{center}
	...
	\begin{tabular}{ l  c  c  c  c  c }
		...
		Phi 					& -     & -    & 1.03  & 1.05 & 1.00\\ %\hline
		Corr.					& -     & -    & 1.00  & 1.00 & 1.00\\ %\hline
		Ratio STD          		& -     & -    & 1.03  & 1.05 & 1.00\\ \hline
		\textbf{3 Ocean, 1 Land}\\
		Phi 					& -     & -    & 0.70  & 0.67 & 0.67\\ %\hline
		Corr.					& -     & -    & 1.00  & 1.00 & 1.00\\ %\hline
		Ratio STD          		& -     & -    & 0.70  & 0.68 & 0.67\\ \hline
		\textbf{1 Ocean, 3 Land}\\
		Phi 					& 1.13  & 1.10 & 1.02  & 0.94 & 0.86\\ %\hline
		Corr.					& 1.00  & 1.00 & 1.00  & 1.00 & 1.00\\ %\hline
		Ratio STD          		& 1.13  & 1.10 & 1.02  & 0.94 & 0.86\\ \hline
		\textbf{3 Ocean, 3 Land}\\
		Phi 					& 1.16  & 1.11 & 1.00  & 0.96 & 0.91\\ %\hline
		Corr.					& 1.00  & 1.00 & 1.00  & 1.00 & 1.00\\ %\hline
